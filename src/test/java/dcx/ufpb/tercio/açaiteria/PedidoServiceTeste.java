...
public class PedidoServiceTeste {
    ...
    @Test
    public void testeCadastrarProdutoNoPedido() {
        Produto p1 = new Produto("002", "Copo 500ml Campeao", 14.00);
        List<Produto> meusProdutos = new ArrayList<>();
        meusProdutos.add(p1);

        Pedido pedido = sistema.cadastrarPedido("Carlos", meusProdutos);
        assertEquals(14.00, pedido.getItens().get(0).getPreco());
    }
    ...
    @Test
    public void testeRemoverPedido() {
        Pedido p = sistema.cadastrarPedido("Tercio", pedidos);
        boolean removido = sistema.removerPedido(p.getIdPedido());

        assertTrue(removido);
        assertEquals(0, sistema.quantidadeTotalPedidos());
    }
}
